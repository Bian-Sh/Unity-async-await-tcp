    ...
    public static partial class UnityAsyncExtensions
    {
        ...
        public struct ResourceRequestAwaiter : ICriticalNotifyCompletion
        {
            ...
            public bool IsCompleted
            {
                get
                {
                    Debug.LogError($"{nameof(ResourceRequestAwaiter)}:  IsCompleted Attribute called, Operation is Done = {asyncOperation.isDone}");
            ...
            public UnityEngine.Object GetResult()
            {
                if (continuationAction != null)
                {
                    Debug.LogError($"{nameof(ResourceRequestAwaiter)}: inside get result~");
                    asyncOperation.completed -= continuationAction;
                    continuationAction = null;
                ...
                {
                    Debug.LogError($"{nameof(ResourceRequestAwaiter)}: inside get result v2~");
                    var result = asyncOperation.asset;
                    asyncOperation = null;
                    return result;
                }
